...
#### 步骤二 创建 Worker 、连接 GitHub 并添加环境变量

Cloudflare Dashboard → 计算 → **Workers & Pages** → **创建应用程序** → **连接 GitHub 仓库** → 授权 GitHub → 选择 fork 的 `cf-kimi-api-ts` → 填写环境变量→ **保存并部署**。
...
| 资源 | 位置 | 名称示例 |
|------|------|----------|
| KV | 存储和数据库 → **Workers KV** → 创建 | `CF_KIMI_API` |
| D1 | 存储和数据库 → **D1 SQL 数据库** → 创建 | `cf-kimi-api-logs` |
...
| **B. 写在 wrangler.toml**（推荐） | 在 fork 仓库中编辑 `wrangler.toml`，追加下方内容并提交 | ✅ 绑定持久，重建不丢 |
