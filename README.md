...
Dashboard → **Workers & Pages** → **KV** → 创建命名空间（如 `CF_KIMI_API`），记下 ID。

Dashboard → **Workers & Pages** → **D1** → 创建数据库 `cf-kimi-api-logs`，记下 ID。

**③ 在 fork 里填入 ID**

打开你 fork 的仓库，编辑 `wrangler.toml`，将 `YOUR_KV_NAMESPACE_ID` 和 `YOUR_D1_DATABASE_ID` 替换为上一步记下的 ID。

> 只需在 GitHub 网页上操作，无需本地命令行。
...
→ **保存并部署**。首次部署即带正确的 KV/D1 绑定，后续重新部署也不会丢失。
