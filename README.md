...
仓库不包含 `wrangler.toml`，所有配置通过 Cloudflare Dashboard 完成，绑定不会因重新构建丢失。
...
| 项目名称 | `cf-kimi-api-ts` |
| 生产分支 | `main` |
| 框架预设 | **无**（关键，否则自动检测 Hono 会失败） |
| 构建命令 | 留空 |
| 部署命令 | `npx wrangler deploy --name cf-kimi-api-ts --main src/index.ts --compatibility-date 2024-12-01` |
...
- `wrangler.toml` 和 `.env` 已在 `.gitignore` 中，不会被误提交
